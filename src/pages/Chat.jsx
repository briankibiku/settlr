// FETCHES DIRECTLY FROM KENYA LAW REPORTS BACKEND
...
//       //   "https://settlr-backend-bto2.onrender.com/analyze-case",
//       //   {
...
const Chat = () => {
  ...
  const analyzeCaseAPI = async (injuries) => {
    try {
      // const response = await fetch("http://localhost:8000/query", {
        const response = await fetch("https://settlr-backend-bto2.onrender.com/query", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  // Format the API response into a readable message
  const formatAIResponse = (apiData) => {
    const { answer, similar_cases, case_urls, sources } = apiData;

    let response = `📊 **Case Analysis**\n\n`;

    // Main answer from LLM
    response += `${answer}\n\n`;

    // Similar Cases from Kenya Law
    if (similar_cases && similar_cases.length > 0) {
      response += `───────────────────────────────\n\n`;
      ...
      if (topCases.length > 0) {
        topCases.forEach((caseItem, index) => {
          console.log(caseItem.score);
          response += `**${index + 1}. ${caseItem.title}**\n`;
          if (caseItem.case_number) {
            response += `📋 Case No: ${caseItem.case_number}\n`;
          }
          if (caseItem.court) {
            response += `⚖️  Court: ${caseItem.court}\n`;
          }
          if (caseItem.year) {
            response += `📅 Year: ${caseItem.year}\n`;
          }
          // response += `📊 Relevance: ${(caseItem.score * 100).toFixed(0)}%\n`;
          if (caseItem.url) {
            response += `🔗 View Judgment: ${caseItem.url}\n`;
          }
          response += `\n`;
        });
      } else {
        response += `No highly relevant cases found in Kenya Law database.\n\n`;
      }
    }
    ...
    if (sources && sources.length > 0) {
      response += `───────────────────────────────\n\n`;
      response += `📚 **Source Documents Used:**\n\n`;
      ...
      if (uniqueSources.length > 10) {
        response += `\n... and ${uniqueSources.length - 10} more documents\n`;
      }
    }
              ...
              <div className="flex justify-start">
                <div className="flex gap-3 max-w-3xl">
                  ...
                  <div className="px-5 py-3 bg-white rounded-2xl shadow-md">
                    ...
                    <p className="text-xs text-gray-500">
                      Analyzing case and searching Kenya Law database...
                    </p>
                  </div>
                </div>
              </div>
